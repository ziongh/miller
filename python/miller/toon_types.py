...
def format_symbol_for_toon(symbol: dict[str, Any], max_doc_length: int = 100) -> ToonSymbol:
    ...
    if "signature" in symbol and symbol["signature"]:
        toon["signature"] = str(symbol["signature"])

    if "doc_comment" in symbol and symbol["doc_comment"]:
        doc = str(symbol["doc_comment"])
        if len(doc) > max_doc_length:
            toon["doc_comment"] = doc[: max_doc_length - 3] + "..."
        else:
            toon["doc_comment"] = doc
    ...
    if "end_line" in symbol:
        toon["end_line"] = int(symbol["end_line"])

    if "language" in symbol:
        toon["language"] = str(symbol["language"])
